...
export type ApiClient = {
    ...
    createBlock: (id: string, nonce: string, transaction: string, prevHash: string, hash: string) => Promise<Block>;
...
export const createApiClient = (): ApiClient => {
    return {
        ...
        createBlock:  (id: string, nonce: string, transaction: string, prevHash: string, hash: string) => {
            return axios.post(APIRootPath + "/blocks/new" ,{headers: {'X-Requested-With': 'XMLHttpRequest'},params: {id: id, nonce:nonce, transaction:transaction, prevHash:prevHash, hash:hash}}).then((res) => res.data);
        },
